...
__device__
void subBytes(aesword_t block[4], unsigned char* sbox_shared) {
	for (int c = 0; c < 4; c++) {
		aesword_t t; // Required temp. Compiler error if not!
		t.b[0] = sbox_shared[block[c].b[0]];
		t.b[1] = sbox_shared[block[c].b[1]];
		t.b[2] = sbox_shared[block[c].b[2]];
		t.b[3] = sbox_shared[block[c].b[3]];
		block[c].w = t.w;
	}
}
...
void invSubBytes(aesword_t block[4]) {
	for (int c = 0; c < 4; c++) {
		aesword_t t; // Required temp. Compiler error if not!
		t.b[0] = inv_sbox[block[c].b[0]];
		t.b[1] = inv_sbox[block[c].b[1]];
		t.b[2] = inv_sbox[block[c].b[2]];
		t.b[3] = inv_sbox[block[c].b[3]];
		block[c].w = t.w;
	}
}
...
void aes128_core(aesword_t expandedkey[11][4], aesword_t *data) {
	...
	for (int i = 1; i < 10; i++) {
		subBytes(localdata, sbox_shared); 
		shiftRows(localdata);
